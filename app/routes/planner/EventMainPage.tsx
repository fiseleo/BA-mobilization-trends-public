...
import { data, Link, type LoaderFunctionArgs, type MetaFunction } from 'react-router';
    ...
    return (
        <div className="min-h-screen font-sans bg-linear-to-b from-sky-50 to-neutral-100 dark:from-neutral-900 dark:to-neutral-900 text-neutral-800 dark:text-neutral-200 p-4 sm:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                {currentEvent && (
                    <div className="mb-10 p-6 bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md rounded-xl shadow-lg border border-neutral-200 dark:border-neutral-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Link to={localeLink(locale,"/planner/students")} className="block p-6 bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md rounded-xl shadow-lg border border-neutral-200 dark:border-neutral-700 hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="p-6 bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md rounded-xl shadow-lg border border-neutral-200 dark:border-neutral-700">
                        <h2 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t('ui.pastEvents')}</h2>
                        <div className="mt-3 max-h-48 overflow-y-auto space-y-1 pr-2">
                            {sortedEvents.map(event => (
                                <Link key={event.id} to={localeLink(locale,`/planner/event/${event.id}`)} className="block p-2 rounded-md hover:bg-neutral-500/10 dark:hover:bg-neutral-900/40 text-sm transition-colors">
